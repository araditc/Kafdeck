...
internal sealed class KafkaRecordConsumerFactory
{
    ...
    public IConsumer<byte[], byte[]> Create(string clusterId)
    {
        ...
        var config = new ConsumerConfig
        {
            ...
            ClientId = $"kafdeck-record-reader-{profile.Id}",
            GroupId = $"kafdeck-record-reader-{Guid.NewGuid():N}",
            SecurityProtocol = MapSecurityProtocol(profile.SecurityProtocol),
            EnableAutoCommit = false,
            EnableAutoOffsetStore = false,
            EnablePartitionEof = true,
            AutoOffsetReset = AutoOffsetReset.Error,
            FetchMaxBytes = checked((int)Kafdeck.Core.Records.RecordOperationBudget.HardMaxRawBytes),
            MaxPartitionFetchBytes = checked((int)Kafdeck.Core.Records.RecordOperationBudget.HardMaxRawBytes),
        };
