...
{
    public static IEndpointRouteBuilder MapKafdeckRecordEndpoints(this IEndpointRouteBuilder app, KafdeckOptions options)
    {
        ArgumentNullException.ThrowIfNull(app);
        ArgumentNullException.ThrowIfNull(options);

        app.MapGet("/api/v1/clusters/{clusterId}/topics/{topicName}/partitions/{partition:int}/records", BrowseAsync)
        ...
        app.MapGet("/api/v1/clusters/{clusterId}/topics/{topicName}/partitions/{partition:int}/records/tail", TailAsync)
            .WithName("v03-records-tail")
            .RequireKafdeckAuthorization(AuthorizationAction.ClusterRead, "clusterId")
            .RequireKafdeckAuthorization(AuthorizationAction.TopicRead, "clusterId", "topicName")
            .RequireKafdeckAuthorization(AuthorizationAction.RecordRead, "clusterId", "topicName");

        app.MapGet("/api/v1/clusters/{clusterId}/topics/{topicName}/partitions/{partition:int}/records/export", ExportAsync)
    ...
    private static async Task<IResult> BrowseAsync(
        ...
        RecordFilterService filterService, RecordMaskingService maskingService, CompiledRecordMaskingPolicy maskingPolicy,
        ISecurityAuditSink auditSink, CancellationToken cancellationToken)
    {
        if (!IsConfiguredCluster(options, clusterId)) return ApiResults.Problem(ApiProblemMapper.InvalidClusterId(clusterId));
        try
        {
            ...
            await AuditRecordAccessAsync(http, auditSink, maskingService.BuildReadAudit(safe), cancellationToken).ConfigureAwait(false);
        ...
        IOptions<HttpJsonOptions> jsonOptions, ISecurityAuditSink auditSink, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _ = RecordFilterCompiler.Compile(query.Filter);
        }
        catch (Exception exception) when (IsSafeQueryError(exception))
        {
        ...
        var tailBudget = new RecordTailBudget(
            Math.Min(query.Read.Budget.MaxRecords, RecordTailBudget.HardMaxRecords),
            Math.Min(query.Read.Budget.MaxRawBytes, RecordTailBudget.HardMaxRawBytes),
            query.Read.Budget.MaxDuration,
            query.Read.Budget.MaxRecordsPerSecond);
        ...
        await foreach (var frame in tailService.TailAsync(tailRequest, maskingPolicy.RequiresStructuredValue || query.RequireDecodedValue, cancellationToken).ConfigureAwait(false))
        {
            ...
            {
                var safe = maskingService.Apply(query.Read, frame.Page, maskingPolicy);
                await AuditRecordAccessAsync(http, auditSink, maskingService.BuildReadAudit(safe), cancellationToken).ConfigureAwait(false);
                payload = new RecordTailSafeFrame("records", safe, null);
            }
            else if (frame.Kind == RecordTailFrameKind.KafkaFailure && frame.Failure is not null)
            {
                payload = new RecordTailSafeFrame("error", null, SafeFailure(frame.Failure));
            }
            else if (frame.Kind == RecordTailFrameKind.AdmissionDenied)
            {
                payload = new RecordTailSafeFrame("admissionDenied", null, null);
            }
            else
            {
                payload = new RecordTailSafeFrame("completed", null, null);
            }
    ...
    private static async Task ExportAsync(
        string clusterId, string topicName, int partition, HttpContext http, KafdeckOptions options,
        RecordFilterService filterService, RecordMaskingService maskingService, RecordExportService exportService,
        CompiledRecordMaskingPolicy maskingPolicy, AuthorizationPolicyEvaluator authorizationEvaluator,
        ISecurityAuditSink auditSink, CancellationToken cancellationToken)
    {
        if (!IsConfiguredCluster(options, clusterId))
        {
        ...
        try
        {
            ...
            var identity = OperatorSessionContextFactory.TryCreate(http.User, out var session) && session is not null ? session.Identity : null;
            var legacyDeploymentAuthorized = options.Deployment.Mode != AccessMode.Oidc;
            ...
            var summary = await exportService.ExportAsync(
                safe, exportRequest, authorizationEvaluator, identity, http.Response.Body, cancellationToken,
                legacyDeploymentAuthorized: legacyDeploymentAuthorized).ConfigureAwait(false);
            await AuditRecordAccessAsync(http, auditSink, exportService.BuildExportAudit(safe, summary), cancellationToken).ConfigureAwait(false);
        }
        catch (UnauthorizedAccessException)
        {
            if (!http.Response.HasStarted)
            {
                await WriteProblemAsync(http, new ApiProblemDefinition(
                    ...
                    "record_export_denied"), cancellationToken).ConfigureAwait(false);
            }
        }
        catch (Exception exception) when (IsSafeQueryError(exception))
        {
    ...
    private static async ValueTask AuditRecordAccessAsync(
        HttpContext http, ISecurityAuditSink auditSink, RecordAccessAuditMetadata metadata, CancellationToken cancellationToken)
    {
        var hasSession = OperatorSessionContextFactory.TryCreate(http.User, out var session) && session is not null;
        var principal = hasSession ? SecurityAuditPrincipal.FromOperator(session!.Identity) : SecurityAuditPrincipal.LegacyDeployment;
        var first = metadata.FirstOffset?.ToString(CultureInfo.InvariantCulture) ?? "none";
        var last = metadata.LastOffset?.ToString(CultureInfo.InvariantCulture) ?? "none";
        var reason = metadata.Operation == "record.export"
            ? $"record_export:p={metadata.Partition};first={first};last={last};policy={Uri.EscapeDataString(metadata.PolicyId)};v={metadata.PolicyVersion};format={metadata.ExportFormat};rows={metadata.ExportRowCount};bytes={metadata.ExportByteCount};outcome={metadata.ExportOutcome}"
            : $"record_read:p={metadata.Partition};first={first};last={last};policy={Uri.EscapeDataString(metadata.PolicyId)};v={metadata.PolicyVersion};read={metadata.ReadBudgetOutcome};filter={metadata.FilterBudgetOutcome}";

        await auditSink.WriteAsync(new SecurityAuditEvent(
            DateTimeOffset.UtcNow,
            SecurityAuditEventType.SensitiveRead,
            principal,
            hasSession ? session!.SessionId.Value.ToString("N") : null,
            metadata.ClusterId,
            metadata.TopicName,
    ...
    private static bool IsSafeQueryError(Exception exception) =>
        exception is ArgumentException or FormatException or OverflowException or RecordFilterCompilationException;

    private static IResult InvalidRecordQuery(string detail) => ApiResults.Problem(InvalidRecordQueryDefinition(detail));
    private static ApiProblemDefinition InvalidRecordQueryDefinition(string detail) => new(
        StatusCodes.Status400BadRequest, "urn:kafdeck:problem:invalid-record-query", "Invalid record query", detail, "invalid_record_query");

    private static async Task WriteProblemAsync(HttpContext http, ApiProblemDefinition problem, CancellationToken cancellationToken)
    {
        http.Response.StatusCode = problem.Status;
        http.Response.ContentType = "application/problem+json";
        await http.Response.WriteAsJsonAsync(new { type = problem.Type, title = problem.Title, status = problem.Status, detail = problem.Detail, code = problem.Code }, cancellationToken).ConfigureAwait(false);
    }

    private static object SafeFailure(KafkaFailure failure) => new
    {
        category = failure.Category.ToString(), code = failure.Code, message = failure.SafeMessage, retryable = failure.IsRetryable,
    };
    ...
    private sealed record RecordHttpQuery(RecordReadRequest Read, RecordFilterRequest Filter, bool RequireDecodedValue)
    {
        public static RecordHttpQuery Parse(IQueryCollection query, string clusterId, string topicName, int partition, bool forceForward = false)
        {
            ...
            var direction = forceForward ? RecordReadDirection.Forward : ParseDirection(query["direction"]);
            ...
            var preFilter = new RecordPreFilter(
                ParseNullableLong(query["minimumOffset"]), ParseNullableLong(query["maximumOffset"]),
                ParseNullableTimestamp(query["minimumTimestampUtc"]), ParseNullableTimestamp(query["maximumTimestampUtc"]),
                EmptyToNull(query["keyEquals"]), EmptyToNull(query["keyPrefix"]), headers);
        ...
        {
            if (string.IsNullOrWhiteSpace(value) || value.Equals("json", StringComparison.OrdinalIgnoreCase)) return RecordExportFormat.Json;
            if (value.Equals("ndjson", StringComparison.OrdinalIgnoreCase)) return RecordExportFormat.Ndjson;
            if (value.Equals("csv", StringComparison.OrdinalIgnoreCase)) return RecordExportFormat.Csv;
        ...
        {
            if (ParseNullableLong(query["offset"]) is { } offset) return RecordAnchor.AtOffset(offset);
            if (ParseNullableTimestamp(query["timestampUtc"]) is { } timestamp) return RecordAnchor.AtTimestamp(timestamp);
            return EmptyToNull(query["anchor"])?.ToLowerInvariant() switch
            {
        ...
        private static RecordReadDirection ParseDirection(string? value) => value?.Trim().ToLowerInvariant() switch
        {
        ...
        private static RecordFilterLanguage ParseFilterLanguage(string? value) => value?.Trim().ToLowerInvariant() switch
        {
        ...
        private static IReadOnlyList<RecordHeaderPredicate> CreateHeaderPredicates(IQueryCollection query)
        {
            ...
            if (name is null)
            {
                if (equals is not null || prefix is not null) throw new ArgumentException("headerName is required with a header predicate.");
        ...
        private static long ParseLong(string? value, long fallback) => string.IsNullOrWhiteSpace(value) ? fallback : long.Parse(value, NumberStyles.None, CultureInfo.InvariantCulture);
        private static long? ParseNullableLong(string? value) => string.IsNullOrWhiteSpace(value) ? null : long.Parse(value, NumberStyles.None, CultureInfo.InvariantCulture);
        private static DateTimeOffset? ParseNullableTimestamp(string? value) => string.IsNullOrWhiteSpace(value) ? null : DateTimeOffset.Parse(value, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal);
