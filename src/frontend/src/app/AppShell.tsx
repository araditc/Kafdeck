...
export function AppShell() {
  ...
  return <main aria-labelledby="kafdeck-title">
    <header><div><h1 id="kafdeck-title"><img src="/kafdeck-logo.webp" alt={productName} width={240} height={240} /></h1><p>{productDescription}</p>{operator && <p>Signed in as <strong>{operator.displayName ?? operator.email ?? 'operator'}</strong>{' '}<button type="button" onClick={() => void kafdeckApi.logout()}>Sign out</button></p>}{authenticationRequired && <p><a href="/api/v1/auth/login">Sign in with your identity provider</a></p>}</div><div><label htmlFor="cluster-selector">Cluster</label>{' '}<select id="cluster-selector" value={selectedClusterId} onChange={event => selectCluster(event.target.value)} disabled={loading || clusters.length === 0}>{clusters.map(cluster => <option key={cluster.data.clusterId} value={cluster.data.clusterId}>{cluster.data.clusterId}</option>)}</select>{' '}<button type="button" onClick={() => void loadClusters()} disabled={loading}>Refresh</button></div></header>
