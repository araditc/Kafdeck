...
function ConfigurationView({ entries }: { entries: ConfigurationEntryData[] }) {
  ...
  return (
    <table>
      <thead><tr><th scope="col">Name</th><th scope="col">Value</th><th scope="col">Source</th></tr></thead>
      <tbody>{entries.map(entry => (
        <tr key={entry.name}>
          <th scope="row">{entry.name}</th>
          <td>{entry.isSensitive ? 'Sensitive value redacted' : (entry.value ?? 'Not set')}</td>
...
export function AppShell() {
  ...
  return (
    <main aria-labelledby="kafdeck-title">
      <header>
        <div><h1 id="kafdeck-title">{productName}</h1><p>{productDescription}</p></div>
        <div>
          <label htmlFor="cluster-selector">Cluster</label>{' '}
          <select id="cluster-selector" value={selectedClusterId} onChange={event => selectCluster(event.target.value)} disabled={loading || clusters.length === 0}>
            {clusters.map(cluster => <option key={cluster.data.clusterId} value={cluster.data.clusterId}>{cluster.data.clusterId}</option>)}
          </select>{' '}
      ...
      {selected && <>
        <section id="overview" aria-labelledby="overview-title">
          ...
          <dl><dt>Kafka cluster ID</dt><dd>{selected.data.kafkaClusterId ?? 'Not observed'}</dd><dt>Health</dt><dd>{String(selected.data.health)}</dd><dt>Controller</dt><dd>{selected.data.controllerBrokerId ?? 'Not observed'}</dd><dt>Brokers</dt><dd>{selected.data.brokers.length}</dd></dl>
          {selected.limitations.length > 0 && <aside aria-label="Cluster limitations"><h3>Limitations</h3><ul>{selected.limitations.map((item, index) => <li key={`${item.capability}-${index}`}>{item.capability ?? 'cluster'}: {item.state}{item.reason ? ` — ${item.reason}` : ''}</li>)}</ul></aside>}
        </section>

        <section id="brokers" aria-labelledby="brokers-title">
          <h2 id="brokers-title">Brokers</h2>
          {selected.data.brokers.length === 0 ? <p>No broker metadata is currently observable.</p> : <table><thead><tr><th scope="col">ID</th><th scope="col">Host</th><th scope="col">Port</th><th scope="col">Role</th></tr></thead><tbody>{selected.data.brokers.map(broker => <tr key={broker.brokerId}><th scope="row">{broker.brokerId}</th><td>{broker.host}</td><td>{broker.port}</td><td>{broker.isController ? 'Controller' : 'Broker'}</td></tr>)}</tbody></table>}
        </section>

        <section id="topics" aria-labelledby="topics-title">
          <h2 id="topics-title">Topics</h2>
          <label htmlFor="topic-search">Search topics</label>{' '}
          <input id="topic-search" type="search" value={topicQuery} onChange={event => setTopicQuery(event.target.value)} />
          {topicsLoading && topics.length === 0 && <p role="status">Loading topics…</p>}
          {topicsError && <p role="alert">{topicsError}</p>}
          {!topicsLoading && !topicsError && topics.length === 0 && <p>No topics match the current search.</p>}
          ...
          {topicDetailError && <p role="alert">{topicDetailError}</p>}
          {topicDetail && <article aria-labelledby="topic-detail-title">
            <h3 id="topic-detail-title">Topic: {topicDetail.data.name}</h3>
            <p>{observationText(topicDetail)} · {topicDetail.data.partitionCount ?? topicDetail.data.partitions.length} partitions · {topicDetail.data.offlinePartitionCount} offline · {topicDetail.data.underReplicatedPartitionCount} under replicated</p>
            <h4>Partitions</h4>
            {topicDetail.data.partitions.length === 0 ? <p>No partitions are observable.</p> : <table><thead><tr><th scope="col">ID</th><th scope="col">Leader</th><th scope="col">Replicas</th><th scope="col">ISR</th><th scope="col">Out of sync</th><th scope="col">Health</th></tr></thead><tbody>{topicDetail.data.partitions.map(partition => <tr key={partition.partitionId}><th scope="row">{partition.partitionId}</th><td>{partition.leaderBrokerId ?? 'No leader'}</td><td>{partition.replicaBrokerIds.join(', ')}</td><td>{partition.inSyncReplicaBrokerIds.join(', ')}</td><td>{partition.outOfSyncReplicaBrokerIds.join(', ') || 'None'}</td><td>{partition.healthReasons.length > 0 ? partition.healthReasons.join('; ') : String(partition.health)}</td></tr>)}</tbody></table>}
            <h4>Configuration</h4>
            {topicConfiguration === null ? (topicDetailError ? null : <p role="status">Loading configuration…</p>) : <ConfigurationView entries={topicConfiguration} />}
          </article>}
        </section>
      </>}
    </main>
